...
`data/hadith.json`, `data/wisdom.json`, and `data/motivation.json` are
already included and ready to commit as-is — edit them any time to add
more entries. `hadith.json`/`wisdom.json` need `text`, `ref`, and `mood`
(`"happy"`, `"emotional"`, or `"peace"`). `motivation.json` is a fourth
section shown alongside the verse/hadith/wisdom, but it's *not* a
generic pep-talk pool — every entry is an actual Qur'an ayah or an
authentic hadith, picked specifically for how uplifting it is, so
picking a mood surfaces a second piece of real Qur'an/Sunnah content
meant to lift the reader, not just inform them. Entries take `text`,
`ref`, `mood`, and an optional `arabic` field (shown above the
translation when present, same as the main verse).
...
currently selected for the hadith/wisdom/motivation picks.

## Translate and Listen

Every section (verse, hadith, wisdom, motivation) has a **Listen**
